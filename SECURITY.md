...
- **v2 `account.json` migration** (`load_account_meta`, `altergo.py:1098-1122`). Legacy accounts without `account.json` are treated in memory as `{"version": 2, "provider": "claude"}` on every read. The file is not rewritten until the next `configure_account` explicitly calls `save_account_meta` — read-only sessions leave legacy accounts on disk unchanged.
