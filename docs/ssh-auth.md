...
`keychain` (the default since v0.45.0) gives each altergo account its own `login.keychain-db`. altergo stores the keychain's unlock password as an entry in your **main** login keychain (with `/usr/bin/security` on the entry's ACL via `-T`), so at launch `/usr/bin/security` can read that password and unlock the per-account keychain. The very first time after the entry is created, macOS may show a **one-time "Always Allow" dialog** at the desktop asking whether security can access the partition this entry lives in; click **Always Allow** once and you'll never see it again from any context (including SSH). After that one-time approval, unlocks are silent on both desk and SSH.

> **Why no upfront password prompt during `--config`?** Earlier versions used `set-generic-password-partition-list` to pre-pin the partition so the desktop dialog never appeared. That command requires the user's macOS login password (interactive prompt with a 10-second timeout — a hard crash if you hesitated), so we removed it. The trade-off is one harmless "Always Allow" click on first launch instead of a mandatory password prompt during configuration. SSH access doesn't depend on the dialog being clicked, because altergo offers an OAuth token bridge (next section) that bypasses the keychain entirely.
